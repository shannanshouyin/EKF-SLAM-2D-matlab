...
sim_time = 60;             % 总仿真时间
dt = 0.5;                  % 时间步长
process_noise = diag([0.001, 0.001]);        % 过程噪声
...
% 存储历史轨迹
model_path = model_state(1:2);
true_path = true_state(1:2);
est_path = est_state(1:2);
...
for t = 0:dt:sim_time
    ...
    %% EKF更新步骤
    [est_state, covariance] = Iekf_update_S(est_state, covariance, measurements, measure_noise);
    
    %% 记录轨迹
    model_path = [model_path, model_state(1:2)];
    true_path = [true_path, true_state(1:2)];
    est_path = [est_path, est_state(1:2)];
