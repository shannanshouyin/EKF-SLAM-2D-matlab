...
state = zeros(2*num_landmarks+3,1);
...
for t = 1:sim_time 
    ...
    rmse_true_model_pos(t) = sqrt(mean((true_path(1:2,t) - model_path(1:2,t)).^2));
    
    rmse_true_EKF_theta(t) = sqrt(mean((true_path(3,t) - est_path(3,t)).^2));
    rmse_true_model_theta(t) = sqrt(mean((true_path(3,t) - model_path(3,t)).^2));
end

figure; hold on;
plot(1:sim_time, rmse_true_EKF, 'r-o','DisplayName', 'EKF-Pose-Error','LineWidth', 2);
plot(1:sim_time, rmse_true_model, 'b-o','DisplayName', 'Model-Pose-Error','LineWidth', 2);
xlabel('time'); ylabel('RMSE(m)'); 
title('EKF与模型位姿RMSE对比');
...
hold on;
plot(1:sim_time, rmse_true_model_pos, 'b-o','DisplayName', 'Model-Position-Error','LineWidth', 2);
xlabel('time'); ylabel('RMSE(m)'); 
title('EKF与模型坐标RMSE对比');
legend('Location', 'northeast');
subplot(2,1,2);
plot(1:sim_time, rmse_true_EKF_theta, 'r-o','DisplayName', 'EKF-Heading-Error','LineWidth', 2);
hold on;
plot(1:sim_time, rmse_true_model_theta, 'b-o','DisplayName', 'Model-Heading-Error','LineWidth', 2);
xlabel('time'); ylabel('RMSE(m)'); 
title('EKF与模型朝向RMSE对比');
legend('Location', 'northeast');


EKF_error_mean = mean(rmse_true_EKF);
Model_error_mean = mean(rmse_true_model);
disp("EKF位姿平均RMSE误差为：");
disp(EKF_error_mean);
disp("模型位姿平均RMSE误差为：");
disp(Model_error_mean);

EKF_error_position = mean(rmse_true_EKF_pos);
Model_error_position = mean(rmse_true_model_pos);
disp("EKF坐标平均RMSE误差为：");
disp(EKF_error_position);
disp("模型坐标平均RMSE误差为：");
disp(Model_error_position);

EKF_error_heading = mean(rmse_true_EKF_theta);
Model_error_heading = mean(rmse_true_model_theta);
disp("EKF朝向平均RMSE误差为：");
disp(EKF_error_heading);
disp("模型朝向平均RMSE误差为：");
disp(Model_error_heading);