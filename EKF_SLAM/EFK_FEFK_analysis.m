...
measure_noise = diag([0.2^2, (pi/180)^2]);   % 观测噪声
...
% h_est_F = plot(est_state_F(1), est_state_F(2), 'm.-', 'DisplayName', 'FEFK估计路径','LineWidth', 2);
% 
% % 绘制朝向箭头
% arrow_length = 2;
...
%        arrow_length*sin(est_state_F(3)), 'm', 'LineWidth', 2, 'MaxHeadSize', 1, 'DisplayName', 'FEFK估计朝向');
%                       
% % 绘制基础图形                      
...
% h_est_landmarks_F = gobjects(0); % 估计地标的图形句柄
% m_line = gobjects(0); % 估计地标的图形句柄
% m_line_F = gobjects(0); % 估计地标的图形句柄
